...
const setCachedData = (data: CryptoCoin[], page: number) => {
  ...
  try {
    ...
    localStorage.setItem(`${CACHE_TIMESTAMP_KEY}-${page}`, Date.now().toString());
  } catch (error) {
    console.error('Error writing to cache:', error);
  }
};

const fetchCryptoData = async (page: number): Promise<CryptoCoin[]> => {
  const COINGECKO_API = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&sparkline=true&page=${page}&per_page=10&order=market_cap_desc`;
...
export const useCryptoData = (page: number = 1) => {
  return useQuery({
    ...
    staleTime: 25000,
    retry: 2,
    initialData: isClient ? getCachedData(page) || undefined : undefined,
  });
};
